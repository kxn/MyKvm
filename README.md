...
当前工程已完成 CH9329 协议与输入核心、传输无关的 RFB 3.8 协议核心、使用模拟 BGRA 帧源的单客户端 RFB TCP 库闭环、en-US RFB keysym 到 HID 的状态化键盘映射，以及 RFB 绝对指针状态映射。RFB 事件泵、断线 `release_all`、真实串口、真实视频采集、可直接运行的无头进程、WebSocket/noVNC 和桌面界面仍按阶段计划继续实现。

当前 `ipkvm-headless` 二进制仍是脚手架，尚不能控制真实机器；键盘和指针映射器已经过自动化测试，但尚未接到 RFB TCP 事件泵和真实串口。
...
- `ipkvm-headless`：已有单客户端 RFB TCP 库接口、en-US RFB 键盘映射器和绝对指针映射器；事件泵、可运行后台进程、HTTP、WebSocket/noVNC 和设备会话仍待实现。
