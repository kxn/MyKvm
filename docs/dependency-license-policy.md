...
CI 发布（`dev-release`/`release` workflow，见 README「发布」章节）自动生成并附带
`THIRD_PARTY_LICENSES.txt`（名称/版本/许可证/来源，由 `cargo metadata` 产出），但
许可证全文、NOTICE 声明、逐文件版权与分发材料仍需在发版时人工核对，机器清单不构成
法律意见。
