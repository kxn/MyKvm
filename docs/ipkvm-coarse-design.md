...
- 当前映射器保证 en-US 可打印 ASCII、左右修饰键、F1-F20、方向键、导航键、系统键和小键盘常用键。
...
- 完成 en-US RFB keysym 到 HID 的状态化键盘映射，覆盖可打印 ASCII、Shift 合成和抑制、左右修饰键、导航键、F1-F20、系统键、数字小键盘、别名引用和确定性错误。
...
- 桌面绝对指针发送最小间隔 8ms（约 120Hz）并做位置/掩码去重；按键状态变化立即发送。
...
- 桌面正式入口是 `ipkvm-desktop-iced`。视频区内鼠标按下进入远程输入模式；
  单纯移动不进入。视频区外按下、Ctrl+Alt+K 或窗口失焦退出远程输入模式。
- 退出远程模式：点击视频外本地 UI、按 Ctrl+Alt+K（本地拦截，不转发远端）、
  或窗口失焦（Alt+Tab 切走）。退出时执行 release_all 并复位本地输入状态。
- 应用退出菜单在关闭窗口前也必须先走远程输入退出路径，避免窗口销毁时遗留远端按键或按钮。
- 桌面控制器事件提交无损：事件通道满时暂存待发送队列，通道恢复后按 FIFO
  补发，不丢按键 down/up；发送失败在状态栏可见。
- 鼠标支持绝对/相对两种模式：相对模式锁定并隐藏本地光标，按帧像素增量发送，
  不依赖目标机分辨率（BIOS/启动菜单用）；Ctrl+Alt+M 切换并在线同步输入泵；
  灵敏度可在高级设置调整（默认 1.0）。
- 当前连接的 `MouseProfile` 是用户选择事实源，`MouseMode` 是解析结果。状态栏选择、
  连接模态、快捷键、加载 profile 和恢复默认值共用同一个当前连接事务：先等待
  活动输入泵确认模式切换，失败不提交 UI profile/mode；如果失败发生在远端
  `release_all` 已发送之后，必须退出远程输入并清空本地捕获，避免本地仍认为
  按键/按钮按下而远端已经释放。成功后清空本地按钮、
  `last_modifiers`、相对采样器、滚轮累计、相对源、绝对去重/限频状态，并同步光标
  隐藏和裁剪。
- 设置模态只修改默认连接，不影响当前会话；连接模态和 profile 加载会影响当前连接。
  菜单打开模态与直接打开模态共用同一字段同步入口，避免 `mouse_profile` 与
  `mouse_mode` 显示分叉。
- 鼠标事件遵循严格模式：绝对模式下移动、按钮、滚轮和释放均走绝对报告；相对模式下均走相对报告。
- Windows 相对模式使用 Raw Input 隐藏消息窗口。启动失败必须清理全局 sender、
  已创建窗口和窗口类，保证下一次可重试；停止时投递退出消息并 join 线程。`ClipCursor`
  仅在前台窗口属于当前进程时应用，避免焦点时序中裁剪到其他应用窗口；后续若 iced
  暴露真实窗口句柄，应进一步绑定到实际主窗口。
