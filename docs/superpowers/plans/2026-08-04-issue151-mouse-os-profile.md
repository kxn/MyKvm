...
- [ ] 为 `MouseProfile::{Windows,Linux,Bios,Android,MacOs,RawAbsolute,RawRelative}` 增加稳定标识、解析和 `MouseMode` 映射测试。
- [ ] 实现 `as_str`, `parse`, `resolve_mode`，未知值返回明确错误；同模式 profile 比较时保留 identity。
- [ ] 运行 `cargo test -p ipkvm-core`。
...
- [ ] 在 `ConnectionSettings` 增加 profile 字段并保留 `mouse_mode` 旧字段读取兼容。
- [ ] 让 `ConnectRequest` 和 session factory 以 profile resolve 后的 mode 构造 sink。
- [ ] 覆盖旧 TOML、新 TOML、缺失/未知 profile、保存/加载 profile 和手动快照。
- [ ] 运行 `cargo test -p ipkvm-desktop-core -p ipkvm-desktop`。
...
- [ ] 将状态栏、连接设置、默认设置和保存/加载 profile 接入同一组选项。
- [ ] 实际模式变化走 controller 的 release/set 顺序；同模式 profile 只更新 identity。
- [ ] 无连接时只更新参数；连接中失败时恢复旧 profile、remote input 和 cursor 状态。
- [ ] 添加应用级测试，覆盖五个 profile、原始模式、失败回滚和同模式不重复切换。
...
- [ ] `WebSettings` 增加 `mouse_profile`，旧 JSON/TOML 仅有 `mouse_mode` 时迁移为 Raw。
- [ ] `POST /api/session` 接受 profile override；增加当前会话切换接口并返回最终 profile/mode。
- [ ] `/api/status` 返回 profile、解析 mode 和本地 capture 状态；服务端串行化切换。
- [ ] Web 连接页、设置和视频状态栏使用相同选项；相对选择不自动请求 Pointer Lock。
- [ ] 添加 HTTP 和 browser fixture 测试，覆盖新旧字段、成功/失败、状态回读。

### Task 5: Documentation

**Files:** `docs/ipkvm-coarse-design.md`, `docs/superpowers/specs/2026-08-04-hid-os-profile-research.md`.

- [ ] 记录 profile 映射、Raw 兼容、Pointer Lock 分层和人工验证矩阵。
- [ ] 运行格式、core/desktop/headless 测试后提交 `feat: add mouse OS profiles (#151)`。
