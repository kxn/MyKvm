...
- 日期：2026-08-02
- 状态：设计定稿；#30/#31 已完成，#32 运行时设备选择已按会话级重启模型接线，#33 待实现
...
> **状态：已接线**（issue #32，运行时换设备采用会话级重启；不做旧 RFB 连接无缝迁移）
...
| `/api/session` | POST | 创建/重启会话（`{video, serial}`），重启=停止并销毁旧会话、释放旧帧源/串口 sink、重建帧源+串口+输入泵，HTTP/TCP/WS 监听服务不动；旧 RFB 连接不承诺无缝迁移，但必须释放 gate 供新连接重连 |
| `/api/status` | GET | 扩展：+ 输入统计、最后输入时间、丢帧计数、串口统计、活动控制者（现有帧源+控制器保留）；读取前刷新丢帧与串口队列统计 |
...
- 单元：配置 merge 优先级、设备枚举格式化、鉴权中间件放行/拒绝、会话替换启动。
- 集成：`/api/devices`、`/api/session` 创建/重启、运行时切换视频源后 `/api/status` 读取新帧源、活动 TCP/WS 连接期间重启后新连接可重新握手、重启前释放旧独占资源、`/api/status` 扩展字段、鉴权拒绝非本机/token 错误。
- 浏览器闭环回归保留。
...
- 真正不断线的帧源热替换、旧 RFB 连接迁移或强踢客户端协议扩展留后续；当前产品能力是运行时选择设备并以会话级重启方式生效。
