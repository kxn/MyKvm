...
前置单清单（排计划时一起排）：#133（位序）、#134（相对指针链路）、#135（设置 API）、
#136（手动停止标记）。

## 12. 建议里程碑（供后续 writing-plans 拆分）

- 前置批次：#133 位序统一 → #136 手动停止 → #135 设置 API → #134 相对指针链路
  （后端协议 + noVNC patch）；
- 主实施批次 A：工具栏/连接页/视频页/状态轮询/断开/多标签/语言/截图（下载+复制）/
  设置弹层/特殊键全量菜单 + 键盘拦截转发/相对模式（pointer lock 接入 #134）；
- 主实施批次 B：错误与恢复展示打磨、浏览器兼容矩阵验收（Chrome/Edge）、
  browser-tests 全量闭环。
