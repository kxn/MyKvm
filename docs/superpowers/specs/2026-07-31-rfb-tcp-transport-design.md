...
- 调用方必须在 `RfbTcpServer::run` 返回前持续消费事件。正确关闭顺序是先发送 shutdown，再继续排空事件直到 `run` 返回，最后丢弃 receiver。
- 如果接收端仍存在但停止消费，而且通道已经满，无损反压会有意推迟连接关闭和 `Disconnected`；server 不绕过队列丢弃生命周期事件。
