...
本设计已经按 issue `#17` 落地，并由 `.\scripts\verify.ps1` 完整验收：

- `third_party/novnc/1.7.0` 保存完整 npm 发布包，固定 tarball 摘要、npm integrity、上游提交和 66 个文件的 SHA-256 清单。
- `include_dir 0.7.4` 在编译时嵌入项目页面和 noVNC 目录；`build.rs` 显式声明两个资源目录的变更触发，避免新增文件未触发 Cargo 重编译。
- `HeadlessWebService::serve` 统一提供页面、许可证、静态模块和现有 `/rfb`，并监督优雅关闭。
- 浏览器夹具使用独立 `browser-fixture` 功能和模拟 BGRA 帧源，默认生产构建不包含该二进制。
- `playwright-core 1.62.1` 驱动系统 Chrome 或 Edge。Node 是浏览器与夹具的唯一进程所有者，PowerShell 只安装锁定依赖并传播退出状态。
- 自动化在 1280×800 和 390×844 视口验证 320×180 四象限帧、无溢出等比缩放、普通按键、缩放坐标、按钮顺序、释放和重连，并拒绝浏览器控制台错误与未知资源回退。

实施没有扩大产品边界：正式 `ipkvm-headless` 主程序仍未组装真实视频、串口和设备生命周期；非回环普通 HTTP 的安全上下文行为仍由后续部署设计处理。
