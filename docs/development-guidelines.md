# 开发规范

本文定义 my_ipkvm 的日常开发规则。目标是让每次改动都能追溯到明确问题、尽量由自动化验证，并把长期结论沉淀到文档中。

## 基本原则

- 所有仓库内自写文档使用中文。
- 非平凡工作以 GitHub issue 为入口。
- 默认优先自动化测试，人工验证只作为无法稳定自动化时的例外。
- 代码修改优先从根因修复，禁止用临时绕过代替真实修复。
- 设计结论、协议约定和开发规则一旦确定，应写入长期文档，而不是只留在 issue 讨论里。

## 文档语言

仓库内由项目维护者编写的文档必须使用中文，包括：

- `README.md`。
- `docs/` 下的设计、计划、审查、规范和说明文档。
- `.github/` 下的 issue 模板、PR 模板和 Actions workflow。
- `AGENTS.md`。

以下内容可以保留原文：

- 外部协议、标准、库文档和许可证。
- 代码标识符、命令、文件路径、协议字段和第三方专有名词。
- 引用外部资料时的短摘录。

外部资料进入 `docs/references/` 时，必须在中文索引或中文说明中写清楚用途、来源和许可证边界。

## Issue 驱动开发

issue 是一次工作的工作单元，记录过程事实；长期文档记录项目长期事实。

需要开 issue 的情况：

- 新功能。
- bug 修复。
- 架构、协议、模块边界或依赖策略调整。
- 测试策略、开发流程或 CI 规则调整。
- 需要硬件或人工验证的问题。

可以不开 issue 的情况：

- 只修正错别字、格式、链接。
- 不改变含义的文档微调。
- 本地实验且不提交到仓库。

每个开发 issue 至少包含：

- 背景和问题。
- 目标和不做范围。
- 验收标准。
- 自动化测试计划。
- 人工验证例外，如果没有则写“无”。
- 相关文档。

分支、提交和 PR 应使用 `#编号` 关联 issue。实现类 PR 必须使用 `Closes #编号` 或等价关键字收口；只有不完成该工作的引用型 PR 才使用 `Refs #编号`。

### 工作完成与收口

非平凡改动只有完成以下收口步骤后才能声称完成：

1. 已有对应 GitHub issue，且 issue 记录了背景、目标、范围、验收标准、测试计划和文档影响。
2. 已按 TDD 要求补充失败测试（适用时），完成实现、回归测试和必要的人工验证。
3. 已创建英文 conventional commit，提交信息包含 `#编号`；不能只修改工作区而不提交。
4. 默认分支开发流程必须从 `main` 创建带 issue 编号的分支，推送 issue 分支、创建 PR，并在 PR 描述中填写 `Closes #编号`、改动摘要、根因或设计依据、测试证据、文档影响和人工验证例外。
5. PR 合并后必须确认 issue 已自动关闭；如果 issue 仍为 open，必须使用 `gh issue close <issue编号> --repo kxn/MyKvm` 关闭，并读回确认状态为 `closed`。
6. 只有在用户明确授权时才允许直接提交或推送 `main`。直接推送不会触发 PR 的 `Closes` 收口，推送成功后必须手动关闭 issue，并读回确认 issue 状态。
7. 收口前必须核对本地 commit 与远端目标分支一致、PR/issue 状态正确、工作区没有误纳入的文件；必要时同步 `HANDOFF.md`、台账和长期文档。

没有完成上述步骤时，只能报告为“实现完成但尚未收口”，不能报告为任务完成。

## Issue 与文档的关系

issue 和文档的职责分开：

- issue 记录一次工作的原因、讨论、取舍、验证和状态。
- 文档记录稳定的设计结论、协议契约、开发规则和用户可依赖的行为。

只改 issue 不改文档的情况：

- 实现细节不影响架构、协议、用户行为和后续开发方式。
- 讨论结论只对当前任务有效。

必须改文档的情况：

- 接口契约变化。
- 模块依赖方向变化。
- 视频、输入、RFB、串口等协议行为变化。
- 用户操作流程变化。
- 测试策略、开发规范或 CI 规则变化。
- 人工验证发现了硬件兼容性限制。

PR 描述必须说明是否修改文档。如果不修改文档，应写明原因。

## 自动化测试优先

默认要求每个改动都有自动化验证。优先级如下：

1. 单元测试：协议帧、状态机、坐标换算、配置选择、错误处理。
2. 集成测试：mock 视频源、fake serial、RFB 协议样例、headless 闭环。
3. 命令级验证：通过 cargo-make 统一入口运行 `cargo make quick`（快速门禁，提交前）或 `cargo make full`（全量门禁，合并前），平台脚本自动选择。快速门禁检查文本编码、锁定依赖图许可证与来源、web 资产、crate 依赖边界、Rust 格式；全量门禁另含全工作区测试、Clippy 和 Rust 文档。粒度命令（`cargo make fmt`/`test`/`clippy`/`doc`）可单独运行。
4. 人工验证：真实采集卡、真实 CH9329/CH340 线、BIOS 行为、操作系统全局快捷键、真实 VNC 客户端兼容性。

人工验证只有在满足以下条件时才允许作为主要证据：

- 自动化环境无法稳定复现。
- 依赖真实硬件、驱动、固件、桌面系统权限或第三方客户端行为。
- issue 或 PR 中写明验证步骤、预期结果和实际结果。
- 能自动化的部分已经自动化。

如果一个人工验证场景后续可以自动化，应开独立 issue 跟踪。

## 根因修复

bug 修复必须优先定位根因。修复说明至少回答：

- 失败路径是什么。
- 根因在哪个模块或契约。
- 为什么这个修复能阻断同类问题。
- 哪个测试覆盖了回归。

禁止把以下做法作为最终修复：

- 固定延时掩盖竞态。
- 吞掉错误或只打印日志。
- 绕过类型系统和状态机。
- 只修改测试来适配错误行为。
- 在调用端到处补特殊分支，而不修正共享抽象。
- 无说明地降低功能或删除校验。

如果必须临时降级，应在 issue 和 PR 中写清楚原因、影响范围、退出条件和后续工作。

## 设计与计划

较大改动应先有设计或计划。判断标准：

- 涉及多个 crate。
- 改变公共 trait、协议帧、配置结构或数据模型。
- 会影响桌面版、headless 版、RFB/Web 入口中的两个或更多入口。
- 需要引入新依赖。
- 需要真实硬件验证。

设计文档写长期决策，计划文档写执行步骤。计划执行完后，若结论改变，应回写设计文档。

大型设计（新 UI、新子系统、协议扩展等）必须**先深入技术调研、后开单**：调研结论先写入
设计文档（至少覆盖现状代码事实、约束与风险、方案取舍、依赖关系），确认后再决定拆单。
禁止在调研完成前按子 feature 预开一批 issue——按子 feature 预开容易丧失关联性，导致
调研漏项或单子之间互相打架。据此拆出的前置单与主实施单必须在文档中列明关联。

## 第三方依赖

新增或升级第三方依赖必须遵守 `docs/dependency-license-policy.md`：

- 先在功能 issue 中说明直接依赖的用途和候选方案。
- 修改后检查实际锁定的直接依赖和传递依赖。
- 宽松许可证按 `deny.toml` 全局白名单自动验证。
- MPL、LGPL 和其他条件许可证必须关联独立 issue，并按具体组件记录链接、修改和分发义务。
- 非 crates.io 来源默认拒绝；例外必须精确到仓库 URL，并固定完整提交 `rev`。
- 禁止用宽泛全局白名单、关闭检查或随意澄清元数据绕过具体依赖审查。

本机固定安装：

```powershell
cargo install --locked --version 0.20.2 cargo-deny
```

当前依赖图审计已接入 `cargo make quick`（快速门禁）与 `cargo make full`（全量门禁）；许可证策略自测（`cargo make license-policy`）在变更 `deny.toml` 时手动运行。noVNC、Qt、FFmpeg、GStreamer、系统 SDK 和静态资源不由 Cargo 检查完整覆盖，必须在引入时单独记录版本、许可证文件和发布义务。

## PR 要求

PR 描述必须包含：

- 关联 issue。
- 改动摘要。
- 根因或设计依据。
- 测试命令和结果。
- 文档影响。
- 人工验证例外。

合并前最低检查（任意平台）：

```
cargo make full
```

开发迭代期间（提交前）可先运行 `cargo make quick` 获得秒级反馈；真实浏览器闭环（`cargo make browser`）与 Windows release 启动冒烟（`cargo make desktop-release`）不包含在门禁链内，涉及对应改动时显式运行。

如果改动不涉及 Rust 代码，也应运行适合范围的检查，例如 Markdown 冲突标记扫描或模板渲染检查。

## GitHub 平台约定

本仓库日常开发在 GitHub 公开仓库 `kxn/MyKvm`（https://github.com/kxn/MyKvm）进行。GitHub 支持仓库内 issue/PR 模板（`.github/`），也支持在 issue、PR、commit 中使用 `#编号` 自动链接引用。

远端 CI 通过 `.github/workflows/verify.yml` 在 GitHub Actions 上运行（Windows 与 Linux 两个 core job，Rust 1.89，通过 `cargo make full` 统一入口调用验证脚本）。两个 core job 覆盖跨平台编译与静态检查，作为 main 分支保护的 required status check；真实浏览器门禁在 GitHub runner 上稳定跑绿前以 advisory 形式存在（continue-on-error），不锁死合并。PR 仍需记录本地 `cargo make full` 的实际结果作为补充证据。

私有 Gitea 仅保留为代码备份和灾难恢复副本（远端 `private`），不在其上开新 Issue/PR。历史 Gitea Issue 编号（#1–#169 等）属私有 Gitea，与 GitHub 新编号无对应关系；历史 commit message 中的 `#编号` 指向 Gitea 旧 Issue。

参考资料：

- GitHub issue 和 PR 模板：https://docs.github.com/communities/using-templates-to-encourage-useful-issues-and-pull-requests
- GitHub 自动链接引用：https://docs.github.com/get-started/writing-on-github/working-with-advanced-formatting/autolinked-references-and-urls
