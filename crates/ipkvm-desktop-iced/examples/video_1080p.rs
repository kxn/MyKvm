...
//!
//! 运行：`cargo run -p ipkvm-desktop-iced --example video_1080p --release -- --duration 10`
...
fn main() -> iced::Result {
    ...
    let stats_for_title = Arc::clone(&stats);
    let stats_for_boot = Arc::clone(&stats);
    ...
    if duration_sec > 0 {
        ...
        std::thread::spawn(move || {
            ...
            // 直接退出（perf 示例可接受跳过优雅停；controller runtime 随进程结束释放）。
            std::process::exit(0);
        });
    }
