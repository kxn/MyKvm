...
use ipkvm_core::{
    Ch9329InputSink, InputError, InputSink, KeyEvent, MouseMode, MouseProfile, PointerButton,
    PointerEvent, SerialCommandQueue,
};
...
impl DesktopSessionFactory<RecordingSink> for MockFactory {
    fn build_video(
        &mut self,
        _request: &ConnectRequest,
    ) -> Result<Arc<dyn FrameSource>, DesktopSessionError> {
        Ok(self.frame_source.clone())
    }
...
impl App<RecordingSink, MockFactory> {
    ...
    pub fn new_mock() -> (Self, Task<Message>) {
        ...
        let status = derive_status(
            controller.should_show_work_view(),
    ...
    F: DesktopSessionFactory<S>,
{
    ...
    pub fn update(&mut self, message: Message) -> Task<Message> {
        match message {
            ...
            Message::PreviewTick => {
                ...
                if self.controller.should_show_work_view() {
                    return Task::none();
                }
            ...
            Message::SetMouseMode(mode) => {
                ...
                self.apply_mouse_profile(profile);
                Task::none()
            }
            Message::SetMouseProfile(profile) => {
                self.apply_mouse_profile(profile);
                Task::none()
            }
            ...
            Message::UiTick => {
                diag::ui_tick();
                if let Err(error) = self.controller.tick_recovery() {
                    self.status_message =
                        Some(t!("message.connect_failed", error = error.to_string()).to_string());
                }
    ...
    fn handle_modal_action(&mut self, action: ModalAction) {
        match action {
            ...
            ModalAction::SetMouseMode(mode) => {
                ...
                self.apply_modal_mouse_profile(profile);
            }
            ModalAction::SetMouseProfile(profile) => {
                self.apply_modal_mouse_profile(profile);
            }
    ...
    /// Profile 应用只负责按设备 id 选中控制器；匹配成功后必须在同一入口
    /// 完成一次真实探测，否则状态会永久停在 Checking，连接按钮也无法恢复。
    fn probe_loaded_control(&mut self) {
        let Some(control_id) = self.selection.selected_control_id.clone() else {
            return;
        };
        let present = self
            .selection
            .control_devices
            .iter()
            .any(|device| device.id == control_id);
        if present && matches!(self.selection.control_status, ControlProbeStatus::Checking) {
            let status =
                self.probe
                    .probe_control(&control_id, self.connection.baud_rate, PROBE_TIMEOUT);
            self.selection
                .record_control_probe(self.connection.baud_rate, status);
        }
    }
    ...
    fn connect_request(&self) -> Option<ConnectRequest> {
        Some(ConnectRequest {
            ...
            mouse_profile: self.connection.mouse_profile,
    ...
    fn handle_iced_event(&mut self, event: iced::Event) {
        ...
        match event {
            ...
            iced::Event::Mouse(iced::mouse::Event::WheelScrolled { delta })
                if self.remote_input =>
            {
                let wheel = wheel_steps(delta);
                if self.connection.mouse_mode == MouseMode::Relative {
                    self.poll_relative();
                    self.flush_relative_pending_with_mask(self.pointer_mask);
                    self.relative_wheel = self.relative_wheel.saturating_add(wheel);
                    ...
                    && let Some(cursor) = self.last_cursor
                {
                    self.send_absolute_wheel(cursor, wheel);
                }
            }
            _ => {}
        }
    ...
    fn sync_cursor(&mut self) {
        ...
        let clip = if grab {
            (*self.video_bounds.borrow()).map(|rect| crate::platform::cursor::ClipRect {
                x: rect.x,
                y: rect.y,
                width: rect.width,
                height: rect.height,
        ...
        if let Some(clip) = clip {
            self.cursor.set_clip_rect(Some(clip));
        } else {
            // 首次布局尚未完成时先释放裁剪；下一次 draw/tick 得到真实
    ...
    fn send_absolute_wheel(&mut self, cursor: iced::Point, wheel: i8) {
        ...
        self.last_pointer_sent = Some((self.pointer_mask, x, y));
        self.last_pointer_sent_at = Some(Instant::now());
    }
    ...
    /// 先消费捕获线程中已经到达的增量，再无视周期取出整数部分，保证
    /// button/wheel 边沿前不会越过尚未发送的移动。
    fn flush_relative_pending_with_mask(&mut self, mask: u8) {
        let Some(rx) = &self.relative_rx else {
            return;
        };
        let mut acc = (0.0f32, 0.0f32);
        ...
        let sensitivity = self.connection.relative_sensitivity;
        let (sx, sy) = crate::input::scale_relative_delta(
            acc.0,
            acc.1,
            self.scale_factor,
            sensitivity,
    ...
    fn connection_view(&self) -> Element<'_, Message> {
        ...
        let refresh = button(text(t!("device.refresh")))
            ...
            .width(Length::Fixed(140.0))
            .height(Length::Fixed(36.0));
        let save_profile = button(text(t!("profile.save")))
            .on_press(Message::OpenModal(ModalKind::SaveProfile))
            .style(crate::theme::secondary_button);
    ...
    fn status_line(&self) -> Element<'_, Message> {
        ...
        let profile_labels: Vec<String> = MouseProfile::ALL
            .into_iter()
            .map(app_mouse_profile_label)
            .collect();
        let profile = PickList::new(
            profile_labels,
            Some(app_mouse_profile_label(self.connection.mouse_profile)),
            |label: String| Message::SetMouseProfile(app_mouse_profile_from_label(&label)),
        )
    ...
    pub fn sync_status(&mut self) {
        let work_view = self.controller.should_show_work_view();
        let online = self.controller.is_control_online();
        if !online && self.remote_input {
            self.exit_remote_input();
        }
        // 诊断：整页切换抖动取证（main_view 按工作会话状态切视频页/连接页）。
        if self.last_diag_online != Some(work_view) {
            diag::log(format!("work_view_flip show={work_view}"));
            self.last_diag_online = Some(work_view);
        }
        self.status = derive_status(work_view, online, self.controller.input_offline_reason());
    }
...
fn app_mouse_profile_from_label(label: &str) -> MouseProfile {
    MouseProfile::ALL
        .into_iter()
        .find(|profile| app_mouse_profile_label(*profile) == label)
...
mod tests {
    ...
    /// 控制泵错误退出后，桌面版应保留工作页并交给共享 supervisor 恢复，
    /// 不再自动销毁整条会话。
    #[test]
    fn pump_error_keeps_work_view_and_reports_control_offline() {
        let (mut app, _) = MockApp::new_mock();
        ...
        *app.recording_sink()
        ...
        app.sync_status();
        assert!(
            app.controller.should_show_work_view(),
            "控制离线不应切回连接页"
        );
        assert!(
            matches!(app.status(), ConnectionStatus::ControlOffline(_)),
            "控制离线应在状态栏展示，而不是变成 Disconnected"
        );
    }

    /// 连续 sync_status 不应把控制离线状态销毁成 Disconnected。
    #[test]
    fn repeated_sync_status_preserves_work_view_after_control_failure() {
        let (mut app, _) = MockApp::new_mock();
        *app.recording_sink()
            .expect("recording sink")
            .fail_next_key
        ...
        app.sync_status();
        assert!(app.controller.should_show_work_view());
        app.sync_status();
        assert!(app.controller.should_show_work_view());
        assert!(matches!(app.status(), ConnectionStatus::ControlOffline(_)));
    }

    /// 用户主动 Disconnect 仍是明确停止，会回到连接页。
    #[test]
    fn user_disconnect_returns_to_connection_page() {
        let (mut app, _) = MockApp::new_mock();
        let _ = app.update(Message::Disconnect);
        assert_eq!(app.status(), &ConnectionStatus::Disconnected);
        assert!(!app.controller.should_show_work_view());
    ...
    #[test]
    fn status_profile_selection_updates_current_mode() {
        let (mut app, _) = MockApp::new_mock();
        let _ = app.update(Message::SetMouseProfile(MouseProfile::Linux));
    ...
    fn settings_modal_updates_default_connection() {
        ...
        assert_eq!(app.modal.mouse_profile, MouseProfile::RawAbsolute);
    ...
    fn toggle_to_absolute_restores_cursor() {
        ...
        assert_eq!(last_visible(&app), Some(false));
        assert_eq!(last_clipped(&app), Some(false));
        // 仍在远程输入时切到绝对：必须恢复光标。
        press_mode_toggle(&mut app);
    ...
    #[test]
    fn status_line_shows_connection_fields_and_profile_selector() {
        let _guard = crate::I18N_TEST_LOCK
            .lock()
            .unwrap_or_else(|p| p.into_inner());
        rust_i18n::set_locale("en");
