...
use ipkvm_video::{
    FrameReceiver, FrameSource, SharedVideoFrame, SourceStatsSnapshot, VideoSourceInfo,
    VideoSourceKind,
};
...
impl FrameHub {
    ...
    pub fn clear(&self) {
        ...
        *self
            .current
            .write()
            .expect("frame hub source lock poisoned") = None;
        self.generation_tx.send_replace(generation);
...
impl FrameHubForwarder {
    pub async fn run(mut self) {
        ...
        if let Some(frame) = source_rx.borrow().clone() {
            self.hub.sender.send_replace(Some(frame));
        }
        loop {
            tokio::select! {
                changed = source_rx.changed() => {
                    ...
                    self.hub.sender.send_replace(source_rx.borrow().clone());
                }
