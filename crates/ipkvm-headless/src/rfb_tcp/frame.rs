...
use super::RfbTcpFrameError;

pub(super) fn frame_view(frame: &VideoFrame) -> Result<BgraFrameView<'_>, RfbTcpFrameError> {
    if frame.pixel_format != PixelFormat::Bgra8888 {
        return Err(RfbTcpFrameError::UnsupportedPixelFormat(frame.pixel_format));
    }

    let width =
        u16::try_from(frame.width).map_err(|_| RfbTcpFrameError::WidthOutOfRange(frame.width))?;
