...
- 已收口：**#99**（黑窗，PR #107）、**#103**（视频居中，PR #108）、**#100**（菜单“断开连接”，PR #109），门禁全绿，issue 已关闭并写验收评论。
- 已知限制：#100 未连接时不置灰（vendored iced_aw 0.14 菜单项无 enabled/disabled 支持，已记录，后续如需置灰补 vendored 补丁）。
- 下一步：**#105**（菜单动作补齐：保存截图 rfd + JPEG、退出、项目主页、复制截图、recent 空态 i18n、paste busy 禁用、语言选中态；含 dialog.rs rfd 收口）→ #102（鼠标输入对齐）→ #104（状态栏）→ #101（对话框对齐 egui）→ #106（语言/字体 Poppins）。
